...
const { saveJSON, generateDateFilename } = require("../utils/utils");
...
async function scrapeMyList(jobId = null) {
  ...
  try {
    ...
    if (vehicles.length > 0) {
      await VehicleController.saveVehicles(vehicles);
      console.log(`📁 My List data saved to database.`);
    ...
    if (isCancelled) {
      ...
      if (vehicles?.length > 0) {
        ...
        await VehicleController.saveVehicles(vehicles);
        console.log(`💾 Partial My List data saved to database`);
        return {
          vehicles,
          filename,
          cancelled: true,
